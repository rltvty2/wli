...
Filesystem strategies:
  ...
  - ext4:   Install the new distro into a loop-device image file on the same partition
             (loopback install), then register it with GRUB via a custom boot entry.

Requirements:
  pip3 install requests
  sudo apt install python3-gi gir1.2-gtk-3.0 gir1.2-vte-2.91 parted btrfs-progs \
                   grub-common grub2-common debootstrap squashfs-tools
"""
...
MIN_BOOT_GB   = 7
MIN_LINUX_GB  = 20
LOOP_IMAGE_DIR = "/var/lib/linux-installer"   # where loop images live on ext4
...
class InstallerWindow(Gtk.ApplicationWindow):
    ...
    def _apply_css(self):
        ...
        * { color: #c8cdd8; }
        ...
        .fs-ext4  { color: #f5a623; font-weight: bold; }
        .fs-other { color: #aaaaaa; }
        .log-box {
            font-family: 'Fira Code', 'Cascadia Code', monospace;
            font-size: 11px; background-color: #0d0f12; color: #87b94a;
        }
        .btn-start {
        ...
        .strategy-ext4  { background-color: #3a2500; color: #f5a623; }
    ...
    def _build_ui(self):
        ...
        sub = Gtk.Label(label="Dual-boot installer  ·  btrfs shrink  ·  ext4 loopback  ·  no USB required")
    ...
    def _build_size_group(self):
        ...
        desc.set_markup(
            ...
            "ext4  → creates a loop image file on same partition"
            "</span>"
        )
    ...
    def _refresh_disk_info(self):
        ...
        if fstype == "btrfs":
            ...
            sc = "strategy-btrfs"
        elif fstype in ("ext4", "ext3", "ext2", "xfs"):
            strat = "STRATEGY: create loop image on same partition (no shrink)"
            sc = "strategy-ext4"
        else:
            strat = f"WARNING: unsupported filesystem ({fstype})"
            sc = "strategy-none"
    ...
    def _ui_set_disk_info(self, text, strat, style_class):
        ...
        for c in ["strategy-btrfs", "strategy-ext4", "strategy-none"]:
            ctx.remove_class(c)
        if strat:
    ...
    def _do_install(self):
        ...
        if fstype == "btrfs":
            ...
            ok = self._strategy_loopback(device, linux_gb, iso_path, distro, distro_key)
        else:
            self.log(f"Unsupported filesystem: {fstype}", error=True)
            self.log("Supported: btrfs, ext4, ext3, ext2, xfs", error=True)
            return
    ...
    # ── ext4 loopback strategy ────────────────────────────────────────────────
    def _strategy_loopback(self, device, linux_gb, iso_path, distro, distro_key):
        """
        1. Create a large sparse image file on the existing partition.
        2. Format it ext4 (as the Linux root).
        3. Mount the ISO and the image, extract the live squashfs into the image.
        4. Install GRUB entry pointing to the loop image.
        5. On first boot the user can optionally migrate to a real partition later.

        This is the 'Wubi-style' approach – no repartitioning required.
        """
        self.log("")
        self.log("━━ Strategy: ext4 loopback image (same partition) ━━")
        self.log("Installing target distro into a loop image file.")
        self.log("No repartitioning required – works on any filesystem.")

        # ── choose image location ──
        _, free = get_partition_info(device)
        if free and free < (linux_gb + 5) * 1_073_741_824:
            self.log(
                f"Not enough free space on {device}. "
                f"Need {linux_gb + 5} GB, have {bytes_to_gb(free)} GB.", error=True)
            return False

        img_dir = Path(LOOP_IMAGE_DIR)
        try:
            img_dir.mkdir(parents=True, exist_ok=True)
        except PermissionError:
            self.log(f"Cannot create {img_dir} – are you root?", error=True)
            return False

        distro_slug = distro_key.lower()
        img_path = img_dir / f"{distro_slug}_root.img"
        iso_mnt  = "/mnt/linux_installer_iso"
        img_mnt  = "/mnt/linux_installer_img"

        for d in [iso_mnt, img_mnt]:
            os.makedirs(d, exist_ok=True)

        # ── create sparse image ──
        self.log(f"Creating {linux_gb} GB sparse image at {img_path}…")
        self.set_status(f"Creating {linux_gb} GB image file…")
        img_bytes = linux_gb * 1_073_741_824
        code, _, err = run(["truncate", "-s", str(img_bytes), str(img_path)])
        if code != 0:
            self.log(f"truncate failed: {err}", error=True)
            return False

        # ── format image ext4 ──
        self.log("Formatting image as ext4…")
        self.set_status("Formatting image ext4…")
        code, _, err = run(["mkfs.ext4", "-F", "-L", distro_slug[:16], str(img_path)])
        if code != 0:
            self.log(f"mkfs.ext4 failed: {err}", error=True)
            return False

        # ── mount image ──
        code, _, err = run(["mount", "-o", "loop", str(img_path), img_mnt])
        if code != 0:
            self.log(f"Cannot mount image: {err}", error=True)
            return False
        ...
        try:
            ok = self._extract_squashfs(iso_mnt, img_mnt, distro, distro_key, img_path)
        finally:
            run(["umount", iso_mnt])
            run(["umount", img_mnt])
        ...
        # ── create GRUB entry ──
        self._create_grub_loopback_entry(img_path, distro, distro_key)

        self._write_boot_instructions(
            strategy="loopback",
            boot_dev=str(img_path),
            linux_dev="(loop image – see instructions)",
            distro_label=distro["label"],
        )
        return True

    def _extract_squashfs(self, iso_mnt, img_mnt, distro, distro_key, img_path):
        """
        Extract the live squashfs root filesystem from the ISO into the image.
        For Debian/Ubuntu-family: casper/filesystem.squashfs or live/filesystem.squashfs
        For Fedora: LiveOS/squashfs.img contains LiveOS/rootfs.img (ext4)
        """
        self.log("Locating squashfs rootfs in ISO…")

        # Possible squashfs locations across distros
        candidates = [
            f"{iso_mnt}/casper/filesystem.squashfs",
            f"{iso_mnt}/live/filesystem.squashfs",
            f"{iso_mnt}/LiveOS/squashfs.img",   # Fedora outer
        ]
        squashfs = next((c for c in candidates if os.path.exists(c)), None)

        if squashfs is None:
            self.log("Cannot find squashfs in ISO.", error=True)
            return False

        self.log(f"Found squashfs: {squashfs}")

        if "LiveOS/squashfs.img" in squashfs:
            # Fedora: squashfs.img → LiveOS/rootfs.img (ext4 image)
            return self._extract_fedora_liveOS(squashfs, img_mnt, img_path)

        # Debian / Ubuntu family: unsquashfs directly into image
        self.log("Extracting squashfs into image (this may take 10–20 minutes)…")
        self.set_status("Extracting filesystem (please wait)…")

        # unsquashfs into a temp dir, then rsync
        tmp_sq = "/mnt/linux_installer_sq"
        os.makedirs(tmp_sq, exist_ok=True)

        code, _, err = run([
            "unsquashfs", "-f", "-d", tmp_sq, squashfs
        ], capture_output=False)   # let it print to terminal for progress

        if code not in (0, 1):    # unsquashfs exits 1 if some files were skipped
            self.log(f"unsquashfs error: {err}", error=True)
            run(["rm", "-rf", tmp_sq])
            return False

        self.log("Syncing extracted root to image…")
        self.set_status("Syncing files…")
        code, _, err = run(["rsync", "-a", "--info=progress2",
                            f"{tmp_sq}/", f"{img_mnt}/"],
        ...
        # Copy kernel + initrd from ISO into image /boot
        self._copy_kernel_from_iso(iso_mnt, img_mnt, distro, distro_key)
        self.log("Squashfs extracted and copied into loop image successfully.")
        return True

    def _extract_fedora_liveOS(self, outer_squashfs, img_mnt, img_path):
        """Fedora: mount outer squashfs.img → extract inner rootfs.img."""
        self.log("Fedora LiveOS: mounting outer squashfs…")
        outer_mnt = "/mnt/linux_installer_outer"
        os.makedirs(outer_mnt, exist_ok=True)

        code, _, err = run(["mount", "-o", "loop,ro", outer_squashfs, outer_mnt])
        if code != 0:
            self.log(f"Cannot mount outer squashfs: {err}", error=True)
            return False

        inner = f"{outer_mnt}/LiveOS/rootfs.img"
        if not os.path.exists(inner):
            run(["umount", outer_mnt])
            self.log("Fedora rootfs.img not found in squashfs.", error=True)
            return False

        self.log("Copying Fedora rootfs.img into loop image via dd…")
        self.set_status("Copying Fedora root filesystem (may take a while)…")
        code, _, err = run(["dd", f"if={inner}", f"of={img_path}",
                            "bs=4M", "conv=notrunc", "status=progress"],
                           capture_output=False)
        run(["umount", outer_mnt])

        if code != 0:
            self.log(f"dd failed: {err}", error=True)
            return False

        # After dd the image IS the Fedora ext4 root – resize to fill allocated space
        run(["e2fsck", "-f", "-y", str(img_path)])
        run(["resize2fs", str(img_path)])
        self.log("Fedora rootfs extracted and resized.")
        return True

    def _copy_kernel_from_iso(self, iso_mnt, img_mnt, distro, distro_key):
        """Copy vmlinuz + initrd into /boot inside the image for GRUB."""
        boot_dir = Path(img_mnt) / "boot"
        boot_dir.mkdir(exist_ok=True)

        pairs = [
            (f"{iso_mnt}/casper/vmlinuz",        f"{boot_dir}/vmlinuz"),
            (f"{iso_mnt}/casper/initrd",          f"{boot_dir}/initrd.img"),
            (f"{iso_mnt}/casper/initrd.lz",       f"{boot_dir}/initrd.img"),
            (f"{iso_mnt}/live/vmlinuz",            f"{boot_dir}/vmlinuz"),
            (f"{iso_mnt}/live/initrd.img",         f"{boot_dir}/initrd.img"),
        ]
        for src, dst in pairs:
            if os.path.exists(src) and not os.path.exists(str(dst)):
                shutil.copy2(src, str(dst))
                self.log(f"Copied {Path(src).name} → {dst}")
    ...
    def _create_grub_loopback_entry(self, img_path, distro, distro_key):
        """
        Write a custom GRUB 2 menu entry that boots the loop image.
        The image is an ext4 root; GRUB's loopback command points to it.
        """
        self.log("Creating GRUB loopback boot entry…")

        grub_custom = Path("/etc/grub.d/40_linux_installer")

        # Detect UUID of device containing the image
        img_device = self._device_of_path(str(img_path))
        code, uuid_out, _ = run(["blkid", "-s", "UUID", "-o", "value", img_device])
        uuid = uuid_out.strip() if code == 0 else ""

        # Relative path from filesystem root
        rel_img = str(img_path)   # absolute path on the host

        # Try to detect kernel/initrd inside the image
        img_mnt_tmp = "/mnt/linux_installer_grub_probe"
        os.makedirs(img_mnt_tmp, exist_ok=True)
        code, _, _ = run(["mount", "-o", "loop,ro", str(img_path), img_mnt_tmp])
        kernel = initrd = ""
        if code == 0:
            for k in sorted(Path(f"{img_mnt_tmp}/boot").glob("vmlinuz*"), reverse=True):
                kernel = "/boot/" + k.name; break
            for r in sorted(Path(f"{img_mnt_tmp}/boot").glob("initrd*"), reverse=True):
                initrd = "/boot/" + r.name; break
            run(["umount", img_mnt_tmp])
        ...
        entry_name = distro["label"].split("–")[0].strip()

        entry_script = f"""#!/bin/sh
# Auto-generated by linux-installer — {datetime.now().strftime('%Y-%m-%d %H:%M')}
exec tail -n +4 $0
menuentry "{entry_name} (loop image)" --class linux {{
    insmod loopback
    insmod ext2
    set root=(loop)
    loopback loop {rel_img}
    set root=(loop)
    linux  (loop){kernel} root=/dev/loop0 ro quiet splash
    initrd (loop){initrd}
}}
"""
        try:
            with open(str(grub_custom), "w") as f:
                f.write(entry_script)
            os.chmod(str(grub_custom), 0o755)
            self.log(f"GRUB entry written: {grub_custom}")
        except Exception as e:
            self.log(f"Could not write GRUB entry: {e}", error=True)
    ...
    def _device_of_path(self, path):
        """Return the block device that contains path."""
        code, out, _ = run(["df", "--output=source", path])
        lines = [l for l in out.strip().splitlines() if not l.startswith("Filesystem")]
        return lines[0].strip() if lines else "/"

    def _write_boot_instructions(self, strategy, boot_dev, linux_dev, distro_label):
...
Linux Installer – Boot Instructions
====================================
Strategy: loopback image (same partition – no repartition)

Distro:     {distro_label}
Image file: {boot_dev}

A GRUB menu entry has been added.  On next boot select:
  "{distro_label.split('–')[0].strip()} (loop image)"

The distro boots from the loop image.  For a permanent installation
you can later copy the image contents to a dedicated partition.
"""
        try:
...
def check_deps():
    missing = []
    for tool in ["parted", "rsync", "mkfs.fat", "mkfs.ext4",
                 "unsquashfs", "btrfs", "blkid", "update-grub"]:
    ...
    if "--check-deps" in sys.argv:
        ...
        if m:
            ...
            print("  sudo apt install " +
                  "parted rsync dosfstools e2fsprogs squashfs-tools btrfs-progs grub-common")
        else:
