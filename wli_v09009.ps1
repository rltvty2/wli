...
if (Test-Path $markerPath) {
    ...
    if ($markerContent -eq "Configured") {
        ...
        # Clean up RunOnce entry if it exists
        try {
...
try {
    ...
    # Get all firmware boot entries
    Write-Host "Retrieving boot entries..." -ForegroundColor Yellow
    
    # Run bcdedit and capture output
    $bcdeditOutput = bcdedit /enum firmware
    
    # Convert output to string array for easier parsing
    $lines = $bcdeditOutput
    ...
    foreach ($line in $lines) {
        # Check for new entry (starts with "Firmware Application")
    ...
    # Find UEFI OS entry (case-insensitive)
    ...
    if ($uefiEntry) {
        ...
        # Execute bcdedit command to set default
        ...
        if ($process.ExitCode -eq 0) {
            ...
            # Schedule automatic restart to boot into Linux
        ...
        # Look for entries containing UEFI
    ...
    # Clean up RunOnce registry entry
    try {
        Remove-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce" -Name "ConfigureLinuxBoot" -ErrorAction SilentlyContinue
...
function Setup-AutoRunScript {
    ...
    try {
        ...
        # Create a marker file to ensure it only runs once
        $markerPath = "$env:ProgramData\LinuxBootConfig\configured.marker"
        "NotConfigured" | Out-File -FilePath $markerPath -Force
        
        # Create only ONE auto-run mechanism - use RunOnce as it's more reliable
        ...
        # Note: No scheduled tasks are created - only the RunOnce registry entry
...
function Download-LinuxISO {
    ...
    # Fixed the string concatenation issue
    $response = [System.Windows.Forms.MessageBox]::Show(
