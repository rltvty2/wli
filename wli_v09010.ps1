...
# Distribution: Linux Mint 22.3 "Zena" (Cinnamon Edition)
...
$form.Text = "USB-less Linux Mint 22.3 Installer for Windows"
...
$headerLabel.Text = "USB-less Linux Mint 22.3 `"Zena`" Installer"
...
$subHeaderLabel.Text = "Cinnamon Edition  |  Installs directly from Windows - no USB required"
...
$statusLabel.Location = New-Object System.Drawing.Point(10, 65)
...
$isoGroup.Text = "ISO Source"
$isoGroup.Font = $normalFont
$isoGroup.Location = New-Object System.Drawing.Point(10, 90)
$isoGroup.Size = New-Object System.Drawing.Size(680, 80)
$form.Controls.Add($isoGroup)

# Download radio button
$downloadRadio = New-Object System.Windows.Forms.RadioButton
$downloadRadio.Text = "Download Linux Mint 22.3 automatically (approx. 2.9 GB)"
$downloadRadio.Font = $normalFont
$downloadRadio.Location = New-Object System.Drawing.Point(20, 25)
$downloadRadio.Size = New-Object System.Drawing.Size(380, 20)
$downloadRadio.Checked = $true
$isoGroup.Controls.Add($downloadRadio)
...
$customRadio.Size = New-Object System.Drawing.Size(130, 20)
...
$customIsoTextbox.Location = New-Object System.Drawing.Point(155, 48)
$customIsoTextbox.Size = New-Object System.Drawing.Size(395, 24)
...
$diskGroup.Location = New-Object System.Drawing.Point(10, 180)
...
$sizeGroup.Location = New-Object System.Drawing.Point(360, 180)
...
$logGroup.Location = New-Object System.Drawing.Point(10, 355)
...
$deleteIsoCheck.Location = New-Object System.Drawing.Point(10, 615)
...
$autoRestartCheck.Location = New-Object System.Drawing.Point(10, 640)
...
$startButton.Location = New-Object System.Drawing.Point(400, 635)
...
$exitButton.Location = New-Object System.Drawing.Point(550, 635)
...
# Enhanced Create-BootConfigScript function for single-run execution
function Create-BootConfigScript {
    ...
    $bootConfigScript = @'
# Set UEFI OS as Default Boot Entry - Single Run Version
# This script must be run as Administrator

param(
    [string]$VolumeLabelPattern = "LINUX_BOOT",
    [string]$DistroName = "Linux"
)

$markerPath = "$env:ProgramData\LinuxBootConfig\configured.marker"

if (Test-Path $markerPath) {
    $markerContent = Get-Content $markerPath -ErrorAction SilentlyContinue
    if ($markerContent -eq "Configured") {
        Write-Host "Boot configuration already completed. Exiting..." -ForegroundColor Green
...
try {
    Write-Host "Starting UEFI boot configuration for $DistroName..." -ForegroundColor Cyan
    Write-Host "Retrieving boot entries..." -ForegroundColor Yellow
    
    $bcdeditOutput = bcdedit /enum firmware
    $lines = $bcdeditOutput
    ...
    Write-Host "`nFound $($bootEntries.Count) boot entries:" -ForegroundColor Cyan
    foreach ($entry in $bootEntries) {
        Write-Host "  ID: $($entry.ID) - Description: $($entry.Description)" -ForegroundColor Gray
    }
    
    $uefiEntry = $bootEntries | Where-Object { $_.Description -like '*UEFI OS*' } | Select-Object -First 1
    
    if ($uefiEntry) {
        Write-Host "`nFound UEFI OS boot entry:" -ForegroundColor Green
        Write-Host "  ID: $($uefiEntry.ID)" -ForegroundColor Cyan
        Write-Host "  Description: $($uefiEntry.Description)" -ForegroundColor Cyan
        
        Write-Host "`nSetting as default boot entry..." -ForegroundColor Yellow
        
        $process = Start-Process -FilePath "bcdedit.exe" -ArgumentList "/set", "{fwbootmgr}", "default", $uefiEntry.ID -Wait -PassThru -NoNewWindow
        
        if ($process.ExitCode -eq 0) {
            Write-Host "Successfully set '$($uefiEntry.Description)' as the default boot entry!" -ForegroundColor Green
            
            "Configured" | Out-File -FilePath $markerPath -Force
            Write-Host "Configuration marked as complete." -ForegroundColor Green
            
            Write-Host "`nVerifying boot configuration..." -ForegroundColor Yellow
            $verifyOutput = bcdedit /enum "{fwbootmgr}"
            
            foreach ($line in $verifyOutput) {
                if ($line -match '(default|displayorder)') {
                    Write-Host "  $line" -ForegroundColor Cyan
                }
            }
            
            Write-Host "`nScheduling restart in 10 seconds..." -ForegroundColor Yellow
            Write-Host "Press Ctrl+C to cancel restart" -ForegroundColor Gray
            
            Start-Sleep -Seconds 10
            
            Write-Host "Restarting to boot into $DistroName..." -ForegroundColor Green
            Restart-Computer -Force
            
        } else {
            Write-Host "Failed to set default boot entry. Exit code: $($process.ExitCode)" -ForegroundColor Red
            Write-Host "You may need to run this script from an elevated PowerShell prompt." -ForegroundColor Yellow
            "Failed" | Out-File -FilePath $markerPath -Force
        }
    } else {
        Write-Host "`nNo boot entry containing 'UEFI OS' was found." -ForegroundColor Red
        
        Write-Host "`nSearching with alternative patterns..." -ForegroundColor Yellow
        
        $exactMatch = $bootEntries | Where-Object { $_.Description -eq 'UEFI OS' }
        if ($exactMatch) {
            Write-Host "Found exact match: $($exactMatch.Description)" -ForegroundColor Green
        }
        
        $uefiEntries = $bootEntries | Where-Object { $_.Description -match 'UEFI' }
        if ($uefiEntries) {
            Write-Host "`nEntries containing 'UEFI':" -ForegroundColor Yellow
            foreach ($entry in $uefiEntries) {
                Write-Host "  - $($entry.Description) (ID: $($entry.ID))" -ForegroundColor Cyan
            }
        }
        
        Write-Host "`nPlease check the exact description of your UEFI OS entry above." -ForegroundColor Yellow
        Write-Host "You can manually set it using: bcdedit /set {fwbootmgr} default {GUID}" -ForegroundColor Gray
        
        "NotFound" | Out-File -FilePath $markerPath -Force
    }
    
} catch {
    Write-Host "An error occurred: $_" -ForegroundColor Red
    Write-Host "Stack trace: $($_.ScriptStackTrace)" -ForegroundColor Red
    "Error: $_" | Out-File -FilePath $markerPath -Force
} finally {
    try {
        Write-Host "`nCleaning up scheduled task..." -ForegroundColor Yellow
        Unregister-ScheduledTask -TaskName "ConfigureLinuxBoot" -Confirm:$false -ErrorAction SilentlyContinue
    ...
    try {
        Remove-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce" -Name "ConfigureLinuxBoot" -ErrorAction SilentlyContinue
        Write-Host "RunOnce registry entry removed." -ForegroundColor Green
    } catch {
        Write-Host "Could not remove RunOnce entry: $_" -ForegroundColor Yellow
    }
    
    Write-Host "`nBoot configuration script completed." -ForegroundColor Cyan
...
# Enhanced Setup-AutoRunScript function for single-run execution
function Setup-AutoRunScript {
    ...
    try {
        ...
        $markerPath = "$env:ProgramData\LinuxBootConfig\configured.marker"
        "NotConfigured" | Out-File -FilePath $markerPath -Force
        
        $runOncePath = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce"
        $runOnceCommand = "powershell.exe -NoProfile -WindowStyle Normal -ExecutionPolicy Bypass -File `"$scriptPath`" -VolumeLabelPattern `"$VolumeLabelPattern`" -DistroName `"$DistroName`""
        
        Set-ItemProperty -Path $runOncePath -Name "ConfigureLinuxBoot" -Value $runOnceCommand -Force
        Log-Message "RunOnce registry entry created for single-run boot configuration"
        Log-Message "Boot configuration will run once after the next restart"
...
function Verify-ISOChecksum {
    ...
    Log-Message "Verifying ISO checksum against known-good Linux Mint 22.3 SHA256..."
    Set-Status "Verifying ISO integrity..."
    
    try {
        $expectedHash = $script:ExpectedChecksum
        ...
        if ($actualHash -eq $expectedHash) {
            ...
            $response = [System.Windows.Forms.MessageBox]::Show(
                "The ISO file checksum does not match the official Linux Mint 22.3 checksum!`n`n" +
...
function Download-LinuxISO {
    ...
    $response = [System.Windows.Forms.MessageBox]::Show(
        ...
        "Click Yes to open the Linux Mint 22.3 download page, No to cancel",
        "Download Failed",
        [System.Windows.Forms.MessageBoxButtons]::YesNo,
...
function Start-Installation {
    ...
    $downloadRadio.Enabled = $false
    $customRadio.Enabled = $false
    $browseButton.Enabled = $false
    
    $distroName = "Linux Mint 22.3"
            ...
            if (Test-Path $script:IsoPath) {
                ...
                try {
                    ...
                    if ($fileInfo.Length -lt 2GB) {
                        ...
                        Set-Status "Re-downloading Linux Mint 22.3 ISO..."
                        if (-not (Download-LinuxISO -Destination $script:IsoPath)) {
                            Log-Message "Failed to download Linux Mint 22.3 ISO!" -Error
                            return
                        }
                    } else {
                ...
                Set-Status "Downloading Linux Mint 22.3 ISO..."
                if (-not (Download-LinuxISO -Destination $script:IsoPath)) {
                    Log-Message "Failed to download Linux Mint 22.3 ISO!" -Error
                    return
                }
...
"@
        ...
        try {
            ...
            if (-not $customRadio.Checked) {
                $validationFile = "$sourceDrive\casper\vmlinuz"
                
                if (-not (Test-Path $validationFile)) {
                    Log-Message "Warning: ISO may not be a valid Linux Mint image (missing expected files)" -Error
                    
                    $response = [System.Windows.Forms.MessageBox]::Show(
                        "The ISO doesn't appear to be a valid Linux Mint 22.3 image. Continue anyway?",
                        "Invalid ISO",
                        [System.Windows.Forms.MessageBoxButtons]::YesNo,
                        [System.Windows.Forms.MessageBoxIcon]::Warning
                    )
        ...
        Log-Message "Copying Linux Mint 22.3 files to $script:NewDrive..."
        ...
        $instructions = @"
UEFI Boot Setup Instructions for Linux Mint 22.3 "Zena"
========================================================

Your Linux Mint 22.3 bootable partition has been created successfully!

Disk Layout:
- Windows C: drive (shrunk)
- Unallocated space: $linuxSizeGB GB (for Linux installation)
- Boot Drive: $script:NewDrive (7 GB, FAT32, labelled LINUXMINT)
- Disk Number: $($script:CDriveInfo.DiskNumber)

Important: The disk now has unallocated space that the Linux Mint 22.3
...
2. The system will then restart again and boot into Linux Mint 22.3
...
5. The system should now boot into Linux Mint 22.3 Live environment

6. During Linux Mint 22.3 installation:
...
Note: The Windows Boot Manager entry was NOT modified to prevent boot issues.
      Use the UEFI boot menu to select between Windows and Linux Mint 22.3.
...
Official Linux Mint 22.3 Resources:
- Release Notes: https://linuxmint.com/rel_zena.php
...
"@
        ...
        $instructions | Out-File -FilePath (Join-Path $env:USERPROFILE "Desktop\LinuxMint22.3_Boot_Instructions.txt") -Encoding UTF8
        
        # Success
        Log-Message "====================================="
        Log-Message "Installation Complete!"
        Log-Message "====================================="
        Log-Message "Linux Mint 22.3 boot partition created at drive $script:NewDrive"
        if ($customRadio.Checked) {
        ...
        Log-Message ""
        Log-Message "The unallocated space is ready for Linux Mint 22.3 installation."
        ...
        if ($autoRestartCheck.Checked) {
            ...
            Log-Message "2. Then it will restart again into Linux Mint 22.3"
            Log-Message ""
        } else {
            Log-Message "*** IMPORTANT BOOT INSTRUCTIONS ***"
            Log-Message "The Windows Boot Manager was NOT modified."
            Log-Message "To boot Linux Mint 22.3, use the UEFI boot menu:"
        ...
        Log-Message "- Desktop\LinuxMint22.3_Boot_Instructions.txt"
        ...
        if ($autoRestartCheck.Checked) {
            ...
            $countdownLabel.Text = "System will restart in 30 seconds...`n`nAfter restart, Windows will boot once more to configure UEFI settings, then restart again into Linux Mint 22.3."
            ...
            $timer.Add_Tick({
                ...
                $countdownLabel.Text = "System will restart in $script:CountdownSeconds seconds...`n`nAfter restart, Windows will boot once more to configure UEFI settings, then restart again into Linux Mint 22.3."
...
$downloadRadio.Add_CheckedChanged({
...
$browseButton.Add_Click({
    ...
    $openFileDialog.Title = "Select Linux Mint 22.3 ISO File"
